...
void enqueue_priority(pcbq_t* q, pcb_t* pcb)
{
    if (q->end == 0)
    {
        ...
        {
            if(q->pcb_queue[i]->priority < pcb->priority)
            {
                for (int j = q->end; j > i; j--)
                {
                    q->pcb_queue[j] = q->pcb_queue[j-1];
                }
                q->pcb_queue[i] = pcb;
                q->end++;
                q->end = q->end%10;
...
int create_process(uint32_t processEntry, uint32_t priority)
{
    ...
    *st = processEntry;
    ...
    enqueue_priority(&queue, tempProcess);
}
...
int main()
{
    ...
    int retval = create_process(&idle, 5); if (retval == 0)
    {
        while (1)
        {
    ...
    retval = create_process(&p1, 10); if (retval == 0)
    {
        while (1)
        {
            char abc[60] = "An error has occurred (Process 1 has failed to be created).";
    ...
    retval = create_process(&p2, 10); if (retval == 0)
    {
        while (1)
        {
    ...
    retval = create_process(&p3, 12); if (retval == 0)
    {
        while (1)
        {
